...
5. 自动生成封面时，需保证 `article-illustrator` 的生图环境可用

## 安装说明

本 skill 适合与 `npx skills add ... --skill wechat-publish` 一起安装到 OpenCode、Claude Code 等支持 `SKILL.md` 的 agent 工具中。
...
1. 读取文章内容并提取标题/摘要
2. 调用 `article-illustrator/scripts/image-gen.ts` 生成单张 16:9 封面图
