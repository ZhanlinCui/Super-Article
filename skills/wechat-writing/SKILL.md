...
inputs:
  ...
  - name: with_illustrations
    ...
    required: false
    default: true
    description: 是否在文章完成后自动配图（调用 article-illustrator 技能）
  - name: with_html_theme
    type: boolean
    required: false
    default: true
    description: 是否输出HTML主题样式文章（调用 markdown2wechat 技能，生成微信公众号内联样式 HTML）
  - name: illustration_density
    type: string
    required: false
    default: balanced
    description: 配图密度（minimal/balanced/rich），传递给 article-illustrator
  - name: illustration_upload
    type: boolean
    required: false
    default: false
    description: 是否将配图上传到七牛云图床并替换为 CDN URL，传递给 article-illustrator
  - name: illustration_aspect_ratio
    type: string
    required: false
    default: "16:9"
    description: 配图宽高比（如 16:9、4:3、1:1），传递给 article-illustrator
...
### Step 6: 自动配图（可选，调用 article-illustrator）
...
**执行方式**：
1. 优先调用 `article-illustrator` 的一键流程脚本：
   ```bash
   node <article-illustrator>/scripts/illustrate-article.ts \
...
### Step 7: HTML 主题样式输出（可选，调用 markdown2wechat）

**触发条件**：`with_html_theme=true`

**目标**：使用 `markdown2wechat` 技能将 Markdown 转换为带微信内联样式的 HTML
...
3. 调用 `markdown2wechat` 技能，执行转换脚本：
     ```bash
     node <markdown2wechat>/scripts/convert.js \
      <input_markdown> \
      <markdown2wechat>/resources/themes/default.css \
      articles/<slug>/article.super.html
     ```
...
**前置条件**：
- `wechat-publish` 技能目录下的 `.env` 已配置 `WECHAT_APP_ID` 与 `WECHAT_APP_SECRET`
- 运行机器的公网 IP 已加入微信公众号后台 IP 白名单
- 若要自动生成封面，发布技能依赖的生图环境也必须可用（由 `article-illustrator` 提供）

**调用方式**：
- 正文必须是带内联样式的 HTML 文件
- 优先使用 Step 7 生成的 `article.super.html`；若不存在则跳过本步骤（或先补执行 Step 7）
- 默认推荐的稳定入口是直接调用 `wechat_draft.js`：
  ```bash
  node <wechat-publish>/scripts/wechat_draft.js \
    --title "文章标题" \
    --file "articles/<slug>/article.super.html" \
    [--author "作者"] \
    [--digest "摘要"] \
    [--thumb "封面media_id"] \
    [--source-url "原文链接"] \
...
- 若希望自动生成封面并发布，也可调用：
  ```bash
  node <wechat-publish>/scripts/publish_with_cover.js \
    --article "articles/<slug>/article.md" \
    --html "articles/<slug>/article.super.html" \
    [--title "文章标题"] \
    [--author "作者"] \
    [--source-url "原文链接"] \
    [--need-open-comment "1"] \
    [--only-fans-can-comment "1"]
  ```
- `<wechat-publish>` 表示当前环境中该技能的安装目录，运行时应以实际路径为准
...
| `缺少 Xiaomi/Gemini/OpenAI API Key` | 自动封面生成依赖的生图环境未配置，检查 `article-illustrator` 相关 `.env` |
| `article.super.html` 不存在 | Step 7 未执行或失败，检查 `with_html_theme=true` |
| 标题过长 | 控制标题 <= 64 字符 |
...
自动配图（article.illustrated.md + illustrations/）
         ↓
HTML 主题样式输出（markdown2wechat → article.super.html）
         ↓
保存到草稿箱（不提交发布）
```
...
html_article_path: articles/how-to-improve-work-efficiency/article.super.html
word_count: 2150
review_score: 92
