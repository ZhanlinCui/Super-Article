...
- `markdown2wechat`：转微信公众号 HTML
- `wechat-publish`：保存到公众号草稿箱

## Install

```bash
npx skills add <repo> --skill wechat-writing
```
...
| Skill | 何时需要 | 作用 |
|---|---|---|
| `article-illustrator` | 需要自动配图时 | 生成插图版文章 |
| `markdown2wechat` | 需要公众号 HTML 时 | 将 Markdown 转为内联 HTML |
| `wechat-publish` | 需要保存到草稿箱时 | 上传 HTML 到微信公众号草稿箱 |
...
- `articles/<slug>/article.super.html`（可选）
...
- 配图：查看 `article-illustrator/README.md`
- 草稿箱发布：查看 `wechat-publish/README.md`
